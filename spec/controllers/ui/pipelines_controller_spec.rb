...
RSpec.describe Ui::PipelinesController, type: :controller do
  ...
  describe "GET #index" do
    ...
    context "regular request" do
      ...
      it "returns the correct response" do
        expect(response).to have_http_status(:success)
        expect(pipelines).to eq(Pipeline.all)
    ...
    context "in progress filter" do
      let!(:pipeline_a) do
        create(:pipeline).tap do |pipeline|
          pipeline.add_graph([Jobs::Base]).first.state = :in_progress
          pipeline.each(&:save!)
        end
      end
      ...
      before do
        create(:pipeline).tap do |pipeline|
          pipeline.add_graph([Jobs::Base]).first.state = :complete
        end
    ...
    context "failed filter" do
      let!(:pipeline_a) do
        create(:pipeline).tap do |pipeline|
          pipeline.add_graph([Jobs::Base, Jobs::Base])
