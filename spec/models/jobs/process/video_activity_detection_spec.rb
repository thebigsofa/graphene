...
RSpec.describe Jobs::Process::VideoActivityDetection do
  subject { create(:job, :video_activity_detection) }
  ...
  let(:tmpfile) { File.open(File.join(tmpdir, "video.json"), "w+") }
  ...
  it "uploads the activity detection json" do
    Timecop.freeze(time) do
      VCR.use_cassette("models/jobs/process/video_activity_detection/ok") do
        subject.process do |response|
          expect(response).to be_kind_of(Excon::Response)
          expect(response.status).to eq(200)

          subject.artifacts["activity_detection"]["video"].delete("scanning_data")
          subject.artifacts["activity_detection"]["video"].delete("source")
